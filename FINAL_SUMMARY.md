...
const isCurlLike = userAgent.includes('curl') || 
                  userAgent.includes('wget') || 
                  !userAgent.includes('mozilla');

if (isCurlLike) {
    window.location.replace('/install.sh');
}
</script>
<!-- HTML页面内容 -->
</html>
...
**工作原理：**
- **浏览器访问**: 显示精美的HTML页面，JavaScript不执行重定向
- **curl访问**: JavaScript检测到命令行工具，自动重定向到安装脚本
...
### 2. 🧠 技术创新
- 创新的智能重定向技术，一个域名支持双重访问
- JavaScript用户代理检测，精准识别访问来源
- 无缝的用户体验切换
