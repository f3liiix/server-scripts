...
interactive_menu() {
    ...
    while true; do
        ...
        echo -e "  ${CYAN}9${NC} ▶ 📦 宝塔面板开心版         ${GRAY}# 安装/升级宝塔开心版(bt.sb)${NC}"
        echo -e "  ${DARK_GRAY}───────────────────────────────────────────────────────────────${NC}"
        echo -e "  ${CYAN}0${NC} ▶ 🚪 退出脚本"
        echo
        
        read -p "$(echo -e "${YELLOW}请输入选择 (0-9): ${NC}")" choice
